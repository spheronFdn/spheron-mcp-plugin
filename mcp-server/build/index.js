...
// YAML parsing helper
const parseYamlEnv = (yamlContent) => {
    try {
...
try {
    ...
    spheronSDK = new SpheronSDK(SPHERON_NETWORK, SPHERON_PRIVATE_KEY);
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    const providerProxyUrl = args.provider_proxy_url || DEFAULT_PROVIDER_PROXY_URL;
    try {
        ...
        switch (operation) {
            case "deploy_compute": {
                ...
                if (args.request) {
                    ...
                    try {
                        ...
                        const response = await axios.post(YAML_API_URL, { request: args.request }, { headers: { "Content-Type": "application/json" } });
                        ...
                        throw new McpError(ErrorCode.InternalError, `YAML generation failed: ${error instanceof Error ? error.message : String(error)}`);
                    }
                }
                else if (args.yaml_content) {
                    // Direct YAML content
                    yamlContent = args.yaml_content;
                    console.error('[API] Using provided YAML content for deployment');
                }
                else if (args.yaml_path) {
                    // YAML from file
                    const yamlPath = args.yaml_path;
                    try {
                        yamlContent = await readFile(yamlPath, 'utf8');
                        console.error('[API] Using YAML content from file:', yamlPath);
                    }
                    catch (error) {
                        throw new McpError(ErrorCode.InvalidParams, `Failed to read YAML file: ${error instanceof Error ? error.message : String(error)}`);
                    }
                }
                else {
                    throw new McpError(ErrorCode.InvalidParams, "Must provide either a natural language request, YAML content, or YAML file path");
                }
                console.error('[API] Creating deployment with Spheron SDK');
                const deploymentResult = await spheronSDK.deployment.createDeployment(yamlContent, providerProxyUrl);
                // Handle BigInt serialization
                const safeResult = JSON.parse(JSON.stringify(deploymentResult, (key, value) => typeof value === 'bigint' ? value.toString() : value));
            ...
            case "fetch_balance": {
                ...
                const decimals = 6;
                const convertUnits = (value) => (BigInt(value) / BigInt(10 ** decimals)).toString() + '.' +
            ...
            case "fetch_deployment_urls": {
                const leaseId = args.lease_id;
                if (!leaseId) {
                    throw new McpError(ErrorCode.InvalidParams, "Lease ID is required");
                }
                ...
                // Handle BigInt serialization
                const safeDetails = JSON.parse(JSON.stringify(deploymentDetails, (key, value) => typeof value === 'bigint' ? value.toString() : value));
                return {
                    content: [{
                            type: "text",
                            text: JSON.stringify({
            ...
            case "fetch_lease_id": {
                ...
                // Handle BigInt serialization
                const safeLeaseDetails = JSON.parse(JSON.stringify(leaseDetails, (key, value) => typeof value === 'bigint' ? value.toString() : value));
                return {
                    content: [{
                            type: "text",
                            text: JSON.stringify({
